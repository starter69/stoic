...
describe Quotation, type: :model do
  ...
  before(:all) do
    @quotation = FactoryGirl.create(:quotation, :tag)
    @tags = @quotation.tags
  end

  #Test setup i.e. FactoryGirl Works with this Model
  it 'has a valid factory' do
    #Given and When (In this case combined)
    fake_factory_girl_generated_quotation_object = FactoryGirl.build(:quotation)
    fake_factory_girl_generated_tag_object = FactoryGirl.build(:tag)
    #Then
    expect(fake_factory_girl_generated_quotation_object).to be_valid
    expect(fake_factory_girl_generated_tag_object).to be_valid
    #Also test the @tags generated by the FactorGirl trait / association on quotation factory
